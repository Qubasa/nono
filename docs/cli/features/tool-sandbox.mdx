...
port-only `AccessNet`, so it works the same on macOS and Linux. Granting the
agent socket into the sandbox as well is redundant, and nono warns about it,
because an agent the sandbox can reach signs for every key it holds.

Under Tool Sandbox the generated `ssh` PATH wrapper is not installed, because
the shim directory owns `ssh` on `PATH`; `GIT_SSH_COMMAND` still points git at
the generated config, and an `ssh` governed by `command_policies` keeps going
through its shim.
...
agent can sign for every key it holds. Under `allow_ssh` there is nothing left
to narrow: no key file and no agent socket exists inside the sandbox, and the
endpoint the credential could be used against is fixed by the allowance. See
