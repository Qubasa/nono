...
A Git profile can model SSH agent as a local socket credential:
...
Reaching an SSH endpoint does not need that grant. `network.allow_ssh`
(`--allow-ssh`) allows one `[user@]host[:port]` endpoint as a mediated SSH
session: the client's session terminates at nono, which authenticates to the
endpoint itself with `--ssh-key` or the host's `SSH_AUTH_SOCK` and relays only
`session` channels. It is host- and port-exact, with the port defaulting to 22,
and it is enforced by the proxy destination pin rather than by Landlock's
port-only `AccessNet`, so it works the same on macOS and Linux. A
session-level grant of the agent socket (`--allow-unix-socket
"$SSH_AUTH_SOCK"`, or `capabilities.unix_sockets` in the profile) is redundant
once `allow_ssh` is active, and nono warns about that one, because an agent the
sandbox can reach signs for every key it holds. The warning inspects the
session capability set only: a Tool Sandbox `local-socket` credential like the
one above builds a fresh capability set per child at launch, long after that
check has run, so it is never covered by the warning.

`network.allow_ssh` together with `command_policies.commands` is refused before
the sandbox starts. A tool-sandboxed child is launched into its own environment
namespace, which strips `NONO_SSH_BASTION` and does not carry
`GIT_SSH_COMMAND`, so the child has no route to the mediation and every SSH
session in it would fail. nono refuses the combination rather than starting a
session with a broken SSH route: drop one or the other. On a profile with no
`command_policies.commands`, the generated `ssh` PATH wrapper is not installed
under Tool Sandbox, because the shim directory owns `ssh` on `PATH`.
...
An agent socket granted this way keeps private keys out of the sandbox, but the
agent can sign for every key it holds, for any host the caller names.
`allow_ssh` does not narrow that. What the mediation bounds is what nono hands
over: nono never puts a key file or an agent socket in the sandbox itself, and
the endpoint *nono's own* credential is used against is fixed by the allowance.
An agent socket the sandbox can reach by some other route (this credential, or
the host's `SSH_AUTH_SOCK` when `AF_UNIX` is unmediated) is outside that bound,
and signs for whatever host the client asks about.

To put the host's agent out of reach, set `linux.af_unix_mediation:
"pathname"`; it is off by default in proxy-only mode, which is the mode
`allow_ssh` selects. See
[SSH Egress Pinning](/cli/features/networking#ssh-egress-pinning).
...
| `Permission denied (publickey)` | Under `allow_ssh`, nono reached the remote server and the credential it holds outside the sandbox was rejected. |
| `Host key verification failed` | The client inside the sandbox rejected nono's per-session host key. A refusal of the *remote* endpoint's key happens outside the sandbox and names the endpoint instead. |
