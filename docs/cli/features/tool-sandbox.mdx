...
A Git profile can model SSH agent as a local socket credential. Reaching the SSH
endpoint itself is a separate, network-side grant: use `network.allow_ssh`
(`--allow-ssh`), which allows one `[user@]host[:port]` endpoint through the
proxy. It is host- and port-exact, with the port defaulting to 22, and it is
enforced by the proxy destination pin rather than by Landlock's port-only
`AccessNet`, so it works the same on macOS and Linux:
...
The agent socket keeps private keys out of the sandbox, but the agent can sign
for every key it holds. To narrow that, grant one dedicated key with
`filesystem.read_file` and configure the client with `IdentitiesOnly=yes`. See
