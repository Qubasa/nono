...
It only ever adds. A profile with no `allow_domain` and no `network_profile` has an open allowlist, and `allow_ssh` leaves it open: the SSH route is wired up and every other destination stays as reachable as it was. Port exactness therefore describes what `allow_ssh` contributes to an allowlist that some other key already activated — writing the endpoint into an empty allowlist would silently make SSH the only thing the session could reach.
...
| Key | What `build.example.com` grants |
|-----|---------------------------------|
| `allow_ssh` | `build.example.com:22` only, and `build.example.com:443` is refused — once something else has activated the allowlist. |
| `allow_domain` | The host on every port — any `:port` suffix is stripped, with a warning that the port is ignored. |

`nono why --host build.example.com --port 443` reports `port_not_allowed` and names the port the host is actually allowed on. Configured endpoints appear in `nono profile show`, `nono profile show --json` (key `allow_ssh` under `network`), `nono profile diff` (also with `--json`), and `nono why --self`.
