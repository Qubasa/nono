...
`allow_ssh` is the only network key in that profile, and that is enough: it activates proxy mode on its own. The session gets a running proxy, `ProxyOnly` network mode, and an active allowlist — everything not allowed is refused.
...
|-----|---------------------------------|
| `allow_ssh` | `build.example.com:22` only. `build.example.com:443` is refused. |
...
When `allow_ssh` is active, nono materialises a generated OpenSSH config in a session-scoped `0700` directory and grants the sandbox read access to it:
