...
|------|-------------|--------------|
| **CONNECT tunnel** | Domain-filtered HTTPS tunneling. Validates the target host against an allowlist, then relays raw TCP bytes. TLS is end-to-end — the proxy never sees plaintext. | `--allow-domain`, `--allow-ssh`, `--network-profile` |
...
`network.allow_ssh` (CLI: `--allow-ssh`, repeatable, reads `NONO_ALLOW_SSH`) allows SSH to specific endpoints through the same CONNECT tunnel. Unlike `allow_domain`, each entry is pinned to one port.
...
`allow_ssh` is the only network key in that profile, and that is enough: it activates proxy mode on its own. The session gets a running proxy and `ProxyOnly` network mode.

Each entry does two things. It **grants** its own `host:port`, and it **pins** that port: every port named by an `allow_ssh` entry becomes closed on every host the entry list does not name. The pin is what makes the allowance mean something on an open policy. A profile with no `allow_domain` and no `network_profile` reaches everything, and adding `--allow-ssh build.example.com` to it does not close HTTPS to anywhere - it closes **port 22** everywhere except `build.example.com`.

```bash
nono run --allow-cwd --allow-ssh build.example.com -- sh -c '
  ssh build.example.com     # reaches the host
  ssh other.example.com     # 403: port 22 is pinned elsewhere
...
Entries are `[user@]host[:port]`. The port defaults to `22`, and any `user@` prefix is accepted and ignored, so an SSH target copied out of a shell command pastes verbatim. IPv6 literals must be bracketed (`[::1]:22`). Wildcards (`*.example.com`), an empty host, a non-numeric port, and a port outside 1-65535 are rejected at profile load time; the error names the offending entry.

| Key | What `build.example.com` does |
|-----|-------------------------------|
| `allow_ssh` | Grants `build.example.com:22`, and closes port 22 on every other host. Other ports are untouched. |
...
```
Host *
  ProxyCommand /path/to/nono ssh-tunnel %h %p
```

`nono ssh-tunnel` is a hidden helper subcommand (an HTTP CONNECT client) that exists for this `ProxyCommand`; it is not a user-facing command. Two environment tweaks point clients at the generated config:
...
<Note>
  The generated config and the PATH wrapper are ergonomics, not enforcement. Invoking `/usr/bin/ssh` by absolute path skips both and fails closed with `EACCES` — bypassing the wrapper never widens access. The enforcement boundary is the kernel destination pin: a seccomp user-notification filter that restricts `connect`/`sendto` to the proxy address.
</Note>
...
| Combination | Reason |
|-------------|--------|
| `--block-net` with `--allow-ssh` | Contradictory: SSH egress pinning requires proxy mode. |
| `--allow-net` with `--allow-ssh` | `--allow-net` bypasses the proxy entirely, so the destination pin could not apply. |
| A host in both `allow_ssh` and `network.no_proxy` | The SSH route must go through the proxy allowlist, not bypass it. |
...
Grant one dedicated SSH key with `filesystem.read_file`, and pass `-o IdentitiesOnly=yes` so the client offers only that key. It has to be a command-line option: the generated config is passed with `ssh -F`, so `~/.ssh/config` is not read inside the session.

```json
{
  "filesystem": { "read_file": ["$HOME/.ssh/id_build"] },
  "network": { "allow_ssh": ["git@build.example.com"] }
}
```

Forwarding `$SSH_AUTH_SOCK` into the sandbox instead (via `filesystem.unix_socket`) weakens the guarantee: the agent can sign for every key it holds, and the sandboxed process can use any of them for the duration of the session. `tool-sandbox-examples/git-ssh/` demonstrates the agent-forwarding setup.
...
| `--allow-ssh <SSH_TARGET>` | Allow one SSH endpoint as `[user@]host[:port]`, port-exact, default port 22 (repeatable). |
