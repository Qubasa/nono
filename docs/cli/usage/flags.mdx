...
The `user@` prefix names the remote user nono authenticates as; without one, nono uses the account name of the uid it runs as, exactly as a plain `ssh host` would. The sandbox does not choose the remote identity, so under `--allow-ssh deploy@build.example.com` a sandboxed `ssh root@build.example.com` still runs as `deploy`. If that uid has no account name, startup fails and the error asks for an explicit `user@` in the allowance rather than guessing.
...
# Non-default port; the user@ prefix picks the remote user. `ssh` has to be
...
nono run --allow-cwd --allow-ssh deploy@other.example.com:2222 \
  -- sh -c 'ssh -p 2222 other.example.com'
```
