...
`filesystem.suppress_save_prompt` in a user profile. The suppress wording is
used deliberately because this flag never turns a denial into an allow rule.
...
Allow SSH to one endpoint, written as `[user@]host[:port]`. Can be specified multiple times. Port-exact: the port defaults to 22 and no other port on that host is opened, unlike `--allow-domain`. Activates proxy mode if not already active.

The route is a mediated SSH session, not a tunnel. The sandboxed client speaks SSH to nono over a session-scoped unix socket, and nono opens its own SSH session to the endpoint and relays it channel by channel. The endpoint is not reachable as raw TCP: `CONNECT build.example.com:22` is refused on the allowed host just as it is everywhere else.

Only `session` channels run, which means `shell`, `exec`, `pty-req`, `window-change`, `subsystem` and `signal`. Every forwarding request is refused with the request type named: `direct-tcpip` (`-L`, `-D`, `-J`), `tcpip-forward` (`-R`), `auth-agent-req@openssh.com` (`-A`) and `x11-req` (`-X`, `-Y`). `sftp`, `scp` and `rsync -e ssh` keep working, because each is a subsystem or a remote command.

The `user@` prefix names the remote user the sandbox may reach the endpoint as, and nono authenticates as that user; without one, it is the account name of the uid nono runs as, exactly as a plain `ssh host` would use. Any other user is refused, not rewritten: under `--allow-ssh deploy@build.example.com` a sandboxed `ssh root@build.example.com` fails with `'root' is not an allowed remote user`. Repeat the flag with another `user@` to allow a second user on the same endpoint. If that uid has no account name, startup fails and the error asks for an explicit `user@` in the allowance rather than guessing.

nono authenticates from outside the sandbox, with `--ssh-key` or the host's `SSH_AUTH_SOCK`, and checks the endpoint's host key against your `~/.ssh/known_hosts` itself. The sandbox needs no grant for a key file or an agent socket.

```bash
# Allow git over SSH to one build host (port 22)
nono run --allow-cwd --allow-ssh build.example.com -- git fetch

# Non-default port; the client must ask for the user the allowance names. `ssh` has to be
# resolved inside the sandbox to pick up the generated config, so go through a
# shell: a top-level `-- ssh …` is resolved against the host PATH beforehand.
nono run --allow-cwd --allow-ssh deploy@other.example.com:2222 \
  -- sh -c 'ssh -p 2222 deploy@other.example.com'
```

In a profile an entry may instead be an object carrying a `commands` list, which restricts that endpoint to those exact command lines and refuses `shell`, `pty-req` and `subsystem` there. See [Per-Endpoint Command Allowlist](/cli/features/networking#per-endpoint-command-allowlist).

Wildcards (`*.example.com`), an empty host, and a port outside 1–65535 are rejected. Incompatible with `--block-net`, with `--allow-net`, and with a Tool Sandbox profile carrying `command_policies.commands`. Also accepted by `nono shell`, `nono why`, and `nono proxy` — but `nono proxy` has no sandboxed child to mediate for, so there it installs the port pin only; a mediated session needs `nono run` or `nono shell`. See [SSH Egress Pinning](/cli/features/networking#ssh-egress-pinning).

Environment variable: `NONO_ALLOW_SSH`

#### `--ssh-key`

Path to a private key file nono authenticates `--allow-ssh` endpoints with, instead of the agent at its own `SSH_AUTH_SOCK`. nono reads and parses the key in the parent process before the sandbox is built, so the file never enters the sandbox and needs no read grant.

```bash
nono run --allow-cwd --allow-ssh git@build.example.com \
  --ssh-key ~/.ssh/id_build -- git fetch
```

An encrypted key file or a hardware-backed `sk-*` key fails at startup with an error naming the file and pointing at `ssh-add`, rather than prompting for a passphrase or failing mid-session. A certificate file also fails at startup, but with a parse error: a certificate is a separate artifact from the private key it belongs to, so it never reaches the key-type check. Put a certificate in your agent, which nono offers alongside the plain keys it holds. The flag applies only to endpoints allowed with `--allow-ssh`, and only where there is a mediated session to authenticate: `nono run`, `nono shell` and `nono wrap` take it, `nono proxy` does not.

Environment variable: `NONO_SSH_KEY`
...
| `--allow-ssh` | `NONO_ALLOW_SSH` | `NONO_ALLOW_SSH=build.example.com` |
| `--ssh-key` | `NONO_SSH_KEY` | `NONO_SSH_KEY=~/.ssh/id_build` |
