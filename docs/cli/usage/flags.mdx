...
Allow SSH to one endpoint through the proxy, written as `[user@]host[:port]`. Can be specified multiple times. Port-exact: the port defaults to 22 and no other port on that host is opened, unlike `--allow-domain`. Activates proxy mode if not already active.

```bash
# Allow git over SSH to one build host (port 22)
nono run --allow-cwd --allow-ssh build.example.com -- git fetch

# Non-default port; any user@ prefix is accepted and ignored. `ssh` has to be
