...
The `user@` prefix names the remote user nono authenticates as; without one, nono uses the username it runs as, exactly as a plain `ssh host` would. The sandbox does not choose the remote identity, so under `--allow-ssh deploy@build.example.com` a sandboxed `ssh root@build.example.com` still runs as `deploy`.
...
Wildcards (`*.example.com`), an empty host, and a port outside 1–65535 are rejected. Incompatible with `--block-net` and `--allow-net`, both of which remove the proxy that pins the destination. Also accepted by `nono shell`, `nono proxy`, and `nono why`. See [SSH Egress Pinning](/cli/features/networking#ssh-egress-pinning).
...
An encrypted key file, a hardware-backed `sk-*` key, or a certificate file fails at startup with an error naming the file and pointing at `ssh-add`, rather than prompting for a passphrase or failing mid-session. The flag applies only to endpoints allowed with `--allow-ssh`.
