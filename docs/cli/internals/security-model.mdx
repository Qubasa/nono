...
| Supervisor denies request | Policy denials: child receives `EACCES`. Supervisor could not mediate the call: `EPERM` | Safe — explicit denial, recorded either way |
| Protected-root match | Request rejected before approval backend is consulted | Safe — protects internal nono state |
| Rate limit exceeded | Approval request automatically denied | Safe — prevents prompt flooding. Applies only to the `openat` approval path; AF_UNIX and proxy decisions are pure capability lookups that never prompt, so they are not rate limited |
...
| Path canonicalization fails | Supervisor denies the call and records the denial under the uncanonicalized path | Safe — fail-closed |
