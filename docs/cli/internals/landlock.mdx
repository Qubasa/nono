...
For compatibility, this mediation is off by default. Turning on proxy-only
network mode (for example with `network.allow_domain`) does not enable it: the
proxy's seccomp filter only forces TCP through the proxy, and Unix-domain IPC
stays governed by the filesystem rules alone. Profiles that need stricter IPC
isolation can opt in:
...
No capability can grant the abstract namespace. Abstract addresses
(`sun_path[0] == '\0'`) and unnamed AF_UNIX addresses carry no filesystem path,
so `filesystem.unix_socket*` cannot describe them. Under `pathname` mediation
abstract `connect(2)` and the `send*` family, and every unnamed address, are
denied with `EACCES` and reported as IPC denials naming the socket kind.
Abstract `bind(2)` passes: it claims a name without reaching any other socket,
and runtimes use it as a crash-safe process lock (omp holds its file locks as
bound `@omp-file-lock-*` datagram sockets). A sandbox that shares the host
network namespace can therefore claim an abstract name before a host service
does. With mediation off, abstract containment is `security.ipc_mode` scoping
above: the default `shared_memory_only` keeps abstract sockets created outside
the sandbox unreachable on V6 kernels, and `ipc_mode: "full"` is the explicit
opt-out. On kernels whose Landlock ABI predates abstract socket scoping, the
scope is reported as requested but not enforced.

Every supervisor-issued AF_UNIX denial leaves a record. Policy denials answer
`EACCES`; `EPERM` means the supervisor could not mediate the call at all (for
example it could not read the address out of the child). A denied socket is
therefore attributable from the session footer without tracing the child.
